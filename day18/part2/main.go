...
func processExpr(stack []*node) int {
	...
	for sp < len(stack) {
		...
		if stack[sp].s == "*" {
			newStack = append(newStack, stack[sp-1], stack[sp])
			sp++
		}
	}
	newStack = append(newStack, &node{isNum: true, val: result})
	// 	log.Printf("newStack: %v", newStack)

	sp = 0
	result = 0
	for sp < len(newStack) {
		if newStack[sp].isNum {
			result = newStack[sp].val
			sp++
			continue
		}
		if newStack[sp].s == "*" {
			result = newStack[sp-1].val * newStack[sp+1].val
			newStack[sp+1].val = result
			sp += 2
		}
	}
