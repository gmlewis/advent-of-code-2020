...
func process(filename string) {
	...
	ingredIsAllergen := map[string]bool{}
	...
	var sum int
	for ingred, count := range allIngredCounts {
		if ingredIsAllergen[ingred] {
			continue
		}
		logf("ingred=%v, count=%v", ingred, count)
		sum += count
	}
...
func (a *alrgnT) findIngredWithAlrgn(foods []*foodT, ingredIsAllergen map[string]bool) string {
	...
	for k, v := range ingCount {
		if ingredIsAllergen[k] {
			logf("Ingredient %v is already identified as an allergen. Skipping.", k)
			continue
		}
		if v == len(a.foods) {
			logf("Found (%v,%v)", k, v)
